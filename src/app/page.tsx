...
export default function ShiftSchedulerPage() {
  ...
  const fetchBalance = useCallback(async () => {
    try {
      const res = await authFetch(`/api/reports?month=${selectedMonth}`);
      ...
      if (savedToken) {
        try {
          ...
          if (data.authenticated && data.user) {
            ...
            if (data.user.region && data.user.region !== "all" && data.user.role !== "admin") {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setUser(userData);
        setIsAuthenticated(true);
        if (data.region && data.region !== "all" && data.role !== "admin") {
  ...
  const generateWeek = async () => {
    ...
    setGenerating(true);
    try {
      const res = await authFetch("/api/schedule", { method: "POST", body: JSON.stringify({ mode: "week", weekStart }) });
      if (res.status === 401) { handleLogout(); return; }
      const data = await res.json();
      if (res.ok) {
  ...
  const exportExcel = async () => {
    ...
    try {
      const res = await authFetch("/api/export", {
        method: "POST",
        body: JSON.stringify({ month: selectedMonth, selectedEmployeeIds: exportSelectedIds.length > 0 ? exportSelectedIds : undefined, dateFrom: exportDateFrom || undefined, dateTo: exportDateTo || undefined }),
      });
      if (res.status === 401) { handleLogout(); return; }
      if (!res.ok) {
        const data = await res.json();
      ...
      <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-950">
            ...
            <div className="flex flex-wrap gap-2 items-center">
              {canEdit && (
                <>
                  ...
                  <Button onClick={() => { setExportSelectedIds([]); setExportDateFrom(""); setExportDateTo(""); setShowExport(true); }} disabled={filteredEntries.length === 0} variant="outline" className="border-emerald-300 text-emerald-700"><FileSpreadsheet className="h-4 w-4 mr-1.5" />Export Excel</Button>
                </>
              )}
              {canAdmin && (
        ...
        <Dialog open={showEmployees} onOpenChange={setShowEmployees}>
          <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
            <DialogHeader><DialogTitle className="flex items-center gap-2"><Users className="h-5 w-5" /> Team Members</DialogTitle><DialogDescription>Manage your team roster</DialogDescription></DialogHeader>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {employees.filter((e) => e.active).map((emp, idx) => {
                      const allHours = employees.filter((e) => e.active).map((_, i) => {
                        const empEntries = filteredEntries.filter((e2) => e2.empIdx === i);
                        return empEntries.reduce((s, e2) => s + e2.hours, 0);
                      });
                      const hrs = allHours[idx] || 0;
                      const avgH = allHours.length > 0 ? allHours.reduce((a, b) => a + b, 0) / allHours.length : 0;
                      const diff = hrs - avgH;
                      const empEntries = filteredEntries.filter((e2) => e2.empIdx === idx);
                      const connHrs = connectionTeam
                        .filter((ct) => {
                          const activeEmps = employees.filter((e) => e.active);
                          const emp = activeEmps[idx];
                          return emp && ct.empName === emp.name;
                        })
        ...
        <Dialog open={showAddConnection} onOpenChange={setShowAddConnection}>
          <DialogContent className="max-w-sm">
            <DialogHeader><DialogTitle className="flex items-center gap-2"><Link2 className="h-5 w-5" /> Assign Connection Person</DialogTitle><DialogDescription>Assign a team member as connection person for a specific week</DialogDescription></DialogHeader>
            <div className="space-y-3 mt-2">
